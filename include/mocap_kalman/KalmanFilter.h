 ...
 *
 * Based on:
 *   Nikolas Trawny and Stergios I. Roumeliotis, "Indirect Kalman
...
/*
 * @brief Error state Kalman filter to estimate the pose and velocity
 *    of rigid bodies tracked by motion capture system
 */
class KalmanFilter
{
  public:
     ...
     * @param freq Expected update frequency
    ...
    /*
     * @brief prepareInitialCondition This function is dedicated
     *    to create the initial condition for the kalman filter. The
     *    function needs to be called twice, once for initializing
     *    the pose and once for the angular and linear velocities
     * @param curr_time_stamp Current time stamp
     * @param m_attitude Measured attitude from the motion capture system
     * @param m_position Measured position from the motion capture system
    ...
    /*
     * @brief prediction Performs the prediction step
